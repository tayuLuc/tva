...
pub fn to_csv(report: &Report, out: &Path) -> Result<()> {
    ...
    for f in &report.frames {
        wtr.write_record(&[f.container_frame.to_string(), f.unique_frame.to_string(), f.streak_length.to_string(), format!("{:.4}", f.real_frame_time_ms), format!("{:.4}", f.instantaneous_fps)])?;
    }
    wtr.flush()?;
    Ok(())
}
