...
impl PixelBuffer {
    ...
    #[must_use] pub fn as_bytes(&self) -> &[u8] { &self.data }
    #[must_use] pub fn into_bytes(self) -> Vec<u8> { self.data }
    #[must_use] pub fn width(&self) -> u32 { self.width }
    #[must_use] pub fn height(&self) -> u32 { self.height }
    #[must_use] pub fn pixel_count(&self) -> usize { (self.width * self.height) as usize }
    #[must_use] pub fn row_bytes(&self) -> usize { self.width as usize * 3 }
    ...
    pub fn split_rgb(&self) -> (Vec<u8>, Vec<u8>, Vec<u8>) {
        ...
        for ch in self.data.chunks_exact(3) { r.push(ch[0]); g.push(ch[1]); b.push(ch[2]); }
        (r, g, b)
    }
}
...
mod tests {
    use super::*;
    #[test] fn valid() { PixelBuffer::new(vec![0u8; 36], 12, 1).unwrap(); }
    #[test] fn bad_size() { assert!(PixelBuffer::new(vec![0u8; 10], 12, 3).is_err()); }
    #[test] fn split() {
